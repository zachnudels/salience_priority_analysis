...
def plot(params):
    ...
    for dependent_variable in params['dependent_variables']:
        fName = os.path.join(params['directories']['root'], f"{dependent_variable}_SMARTdata.p")
        ...
        salient = Line2D([0], [0], color=color1, lw=4, label=f"{dependent_variable} target")
        non_salient = Line2D([0], [0], color=color2, lw=4, label=f"Non-{dependent_variable} target")
        salient_KDE = Line2D([0], [0], color=color1, lw=2, label=f"KDE {dependent_variable} target",
                             linestyle='dashed')
        non_salient_KDE = Line2D([0], [0], color=color2, lw=2, label=f"KDE non-{dependent_variable} target",
                                 linestyle='dashed')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(fig_path, f"fig1_exp_{str(exp)}_{dependent_variable}.png"))
        plt.close()
        ...
        plt.savefig(os.path.join(fig_path, f"fig2_exp_{str(exp)}_{dependent_variable}.png"))
        # plt.show()
        # # saving the figure
